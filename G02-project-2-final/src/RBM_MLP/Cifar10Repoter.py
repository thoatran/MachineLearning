...
class cifarSupervisedReporter:
    ...
    def update(self, epoch, training_error, validation_error, validation_labels, validation_guesses):
        ...
        # Set title:
        title = '%s - Epoch: %d, Training Error: %f, Validation Error: %f' % (
        self.name, epoch + 1, training_error, validation_error)
...
class cifarUnsupervisedReporter:
    def __init__(self, name, report_freq, buffer_size=100):
        self.name = name
        self.report_freq = report_freq
        self.error_buffer = buffer_size
        ...
        self.cifar_plot = gridspec.GridSpecFromSubplotSpec(2, 10, subplot_spec=self.outer_grid[1], hspace=0.05,
                                                           wspace=0.05)
        # Setup error rate subplot:
        self.error_plot.set_xlabel('Epoch')
        self.error_plot.set_ylabel('Error')
        ...
        # Add error legend:
        self.error_legend = self.error_plot.legend(loc='upper left', shadow=True)
        # Initialize cifar images:
        self.cifar_images = []
        for i in range(20):
            arr = np.zeros((32, 32))
            ax = self.fig.add_subplot(self.cifar_plot[i])
            self.cifar_images.append(ax.imshow(arr, cmap='gray', vmin=0, vmax=1, aspect='equal', animated=True))
            ax.set_xticks([])
            ax.set_yticks([])
    ...
    def update(self, epoch, training_error, validation_error, validation_digits, validation_recons):
        ...
        for i in range(10):
            self.cifar_images[i	].set_data( vis_digits[ i, : ].reshape(32, 32) )
            self.cifar_images[ i + 10 ].set_data( vis_recons[ i, : ].reshape(32, 32) )
        # Draw plot:
        if self.is_jupyter:
            self.fig.canvas.draw()
            time.sleep( 0.01 )
